...
function PaymentTable({ mortgage }) {
  ...
  useEffect(() => {
    ...
    let arr = [];
    if (year === startYear) {
      for (let i = 0; i < 12 - start_month; i++) {
        arr.push(mortgage[i]);
      }
    } else if (year === startYear + totalYears) {
      for (let i = mortgage.length - start_month; i < mortgage.length; i++) {
        arr.push(mortgage[i]);
      }
    } else {
      let startIndex = (year - startYear) * 12 - start_month;
    ...
    setCurrentData(arr);
  }, [year, startYear, totalYears, mortgage, start_month]);
